...
public class PlayerMinion {
    ...
    private String key, skin;
    private int action, actions;
    ...
    public PlayerMinion(Player p, Location spawn, String key) {
    ...
    public void firstSpawn() {
        ...
        ArrayList<Entity> entities = new ArrayList<>(spawn.getWorld().getNearbyEntities(spawn, 0.5, 0.5, 0.5));
        ...
        MinionUtils.addMinionUUID(armor.getUniqueId());
        this.enabled = minion.check(spawn);
        loaded = true;
    }
    ...
    public void destroy() {
        ...
        if (armor != null) {
            MinionUtils.removeMinionUUID(armor.getUniqueId());
            armor.remove();
        }
        if (armor != null) {
    ...
    private void executeAction() {
        ...
        if (minion instanceof MinionLumberjack) {
            ...
            if (l != null) {
                s = l;
                locations.put("SAPPLING", new CVector(l.toVector()));
                l.getWorld().playEffect(l.clone().add(0.5, 0.5, 0.5), Effect.HAPPY_VILLAGER, 20);
            } else {
        ...
        if (minion instanceof MinionFarmer) {
            ...
            Location l = mm.checkFamerDirt(spawn);
            if (mm.getPlace().getType().equals(Material.NETHER_WARTS)) {
                l = null;
            }
            if (l != null) {
                s = l;
