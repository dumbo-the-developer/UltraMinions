...
public class PlayerListener implements Listener {
    ...
    private Main plugin;
    private Map<UUID, Long> lastRemove = new HashMap<>(), lastClick = new HashMap<>();
    ...
    public void onInteract(PlayerInteractEvent e) {
        if (e.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
            ...
            ItemStack item;
            if (plugin.getVc().is1_9to15()) {
                ItemStack main = p.getInventory().getItemInHand();
                ...
                ItemStack offff = p.getItemInHand();
                if ((offff.getType().equals(Material.LAVA_BUCKET) || offff.getType().equals(Material.WATER_BUCKET) || offff.getType().isBlock()) && (plugin.getUm().getFuel(false, offff) != null || plugin.getUm().getAutoSell(offff) != null || plugin.getUm().getCompressor(offff) != null || plugin.getUm().getAutoSmelt(offff) != null)) {
                    e.setCancelled(true);
                    p.sendMessage(plugin.getLang().get("messages.noPlaceUpgrades"));
                    return;
                }
                if (plugin.getSkm().getMinionSkinByName(offff) != null) {
                    e.setCancelled(true);
                    p.sendMessage(plugin.getLang().get("messages.noPutSkin"));
                    return;
                }
                if (NBTEditor.getString(offff, "KEY") == null) {
                    return;
                }
                item = offff;
            }
            ...
            PlayerMinion pm = new PlayerMinion(p, b.getLocation().clone().add(0.5, 1, 0.5), key);
            MinionManager.createMinion(item, level, generated, food, health, workTime, sleep, fueltime, pm, plugin);
            pd.addPlayerMinion(pm);
            pm.firstSpawn();
            if (plugin.getVc().is1_9to15()) {
                removeItemInHand(p, mainHand);
            } else {
                removeItemInHand(p);
            }
    ...
    public void loadChunk(Chunk chunk) {
        ...
        if (plugin.getMm().getMinionsToLoad().size() > 0 || plugin.getMm().getMinionsToRemove().size() > 0) {
            ...
            for (Entity ent : entities) {
                ...
                if (MinionUtils.isMinionUUID(uuid)) {
    ...
    private boolean isInBlockRadius(Block b) {
        ...
        for (Entity e : entities) {
            if (e instanceof ArmorStand) {
                if (!checked) {
                    checked = MinionUtils.isMinionUUID(e.getUniqueId());
                }
            }
        }
        return checked;
