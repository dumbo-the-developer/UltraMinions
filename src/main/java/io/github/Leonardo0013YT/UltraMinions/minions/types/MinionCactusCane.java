...
public class MinionCactusCane extends Minion {
    ...
    private List<Vector> woods = Arrays.asList(new Vector(2, 0, 2), new Vector(-2, 0, 2), new Vector(2, 0, -2),
            new Vector(-2, 0, -2), new Vector(2, 0, 0), new Vector(0, 0, 2), new Vector(-2, 0, 0), new Vector(0, 0, -2));
        ...
        if (l != null) {
            if (!plugin.getAdm().isStackable(l)) {
                ...
                if (material.getType().equals(Material.SUGAR_CANE)) {
                    ...
                    buildTree(l, Material.SUGAR_CANE_BLOCK);
                } else {
    ...
    public boolean check(Location spawn) {
        ...
        for (Vector l : woods) {
            ...
            if (p.equals(Material.SUGAR_CANE) && t.equals(Material.GRASS)) {
                continue;
            }
            yes = false;
        }
